...
resourcegroup_name = 'lummy'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'lummy'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
automobile = Entity()
automobile.PartitionKey = 'automobilelisting'
automobile.RowKey = '003'
automobile.make = 'Chevrolet'
automobile.model = 'Tahoe Premier'
automobile.year = '2017'
automobile.color = 'Black'
automobile.price = 58984
table_service.insert_entity('itemstable', automobile)
print('Created entry for Chevy Tahoe...\n')
...
coffee.cupsize = 'Short'
coffee.cupprice = 3.89
table_service.insert_entity('itemstable', coffee)
print('Created entry for a Starbucks Caffe Latte...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'automobilemenu'", select='description,cost')
for item in items:
